// Read-only provider capture. Downloads current v5 source bytes, proves exact byte identity
...
const entries = CURRENT_TEMPLATE_REGISTRY.filter((entry) => entry.version === 5 && entry.orderedSourceAttachments);
if (entries.length !== 2) throw new Error('provider attestation capture requires exactly two current W-2 v5 entries');
